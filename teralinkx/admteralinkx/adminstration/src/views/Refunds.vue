<template>
  <div class="space-y-6 animate-fade-in">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-semibold text-slate-900 dark:text-white">Refunds</h1>
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Manage downtime refunds</p>
      </div>
    ...
    <div v-if="error" class="bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 rounded-lg p-4">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="text-sm font-medium text-rose-800 dark:text-rose-400">Failed to load refunds</h3>
            <p class="text-xs text-rose-600 dark:text-rose-500 mt-1">{{ error }}</p>
          </div>
        </div>
        <button @click="fetchRefunds" class="px-3 py-1.5 bg-rose-600 text-white rounded-lg hover:bg-rose-700 text-sm">Retry</button>
      </div>
    </div>

    <!-- Metrics -->
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 animate-slide-up">
      <ModernMetricCard title="Eligible Clients" :value="stats.eligible_clients" icon="👥" color="blue" />
      <ModernMetricCard title="Total Refunded" :value="`KSh ${formatNumber(stats.total_refunded || 0)}`" icon="💰" color="emerald" :formatted="false" />
      <ModernMetricCard title="Pending" :value="stats.pending_refunds" icon="⏳" color="amber" />
      <ModernMetricCard title="Avg Refund" :value="`KSh ${formatNumber(stats.average_refund || 0)}`" icon="📊" color="purple" :formatted="false" />
    </div>
    ...
    <div class="space-y-4 animate-slide-up" style="animation-delay: 0.1s">
      ...
      >
        <template #cell-status="{ value }">
          <span :class="value === 'completed' ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400' : 'bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-400'" class="px-2 py-0.5 text-xs font-medium rounded-full">
            {{ value }}
          </span>
        </template>
      </DataTable>
...
export default {
  ...
  setup() {
    ...
    const columns = [
      ...
      { key: 'client_username', label: 'Client', sortable: true },
      { key: 'amount', label: 'Amount', sortable: true, format: (v) => `KSh ${v}` },
      { key: 'downtime_minutes', label: 'Downtime (min)', sortable: true },
      { key: 'status', label: 'Status', sortable: true },
      { key: 'created_at', label: 'Date', sortable: true, format: (v) => new Date(v).toLocaleDateString() }
    ]

    const filters = [
      { key: 'status', label: 'Status', options: [{ value: 'completed', label: 'Completed' }, { value: 'pending', label: 'Pending' }] }
    ]
    ...
    return {
      loading, error, refunds, stats, searchTerm, columns, filters, filteredRefunds,
      fetchRefunds, refreshData, handleFilterChange, clearFilters, formatNumber
    }
  }
}
</script>
