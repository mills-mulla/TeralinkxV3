    ...
    <div class="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 z-40 flex items-start justify-center pt-16">
      <div class="bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-full max-w-sm max-h-[calc(100vh-4rem)] overflow-y-auto mx-4">
        ...
        <form @submit.prevent="submitForm" class="px-4 py-3 space-y-3">
          ...
          <!-- Available Coupons -->
          <div v-if="availableCoupons.length > 0" class="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-md p-3 mb-3">
            ...
            <div class="space-y-2">
              <div
                ...
                :class="[
                  'relative overflow-hidden rounded-lg cursor-pointer transition-all duration-300 transform hover:scale-105',
              ...
              >
                <!-- Coupon Card -->
                <div class="bg-gradient-to-r from-purple-500 to-pink-500 p-2 text-white relative">
                  <!-- Decorative circles -->
                  ...
                  <!-- Selection indicator -->
                  <div v-if="selectedCoupon?.code === coupon.code" class="absolute top-0.5 left-0.5">
...
const useCredit = ref(true) // Default to using credit
...
const discountAmount = computed(() => {
  if (selectedCoupon.value) {
    const discount = (originalPrice.value * selectedCoupon.value.discount_value) / 100
    return Math.min(discount, originalPrice.value)
  }
  return 0
})
...
function setPhoneNumber() {
  console.log('Dashboard phone:', dashboardStore.phoneNumber)
  ...
  console.log('Dashboard phone:', dashboardPhone)
  ...
  if (phoneToUse) {
    ...
    console.log('Set phone number to:', phoneNumber.value)
  }
}
